...
class MongoCommand(object):
    ...
    def update_one(self, database, collection, query, set_object=None, options=None):
        ...
        try:
            ...
            self.mylog.error("update_one", query=query, collection=collection, database=database, err=str(exc), exc=exc)
            return None
    ...
    def estimated_document_count(self, database, collection, query):
        try:
            if not query:
                query = {}
            return self._mongo[database][collection].estimated_document_count(query)
        except Exception as exc:
            self.mylog.error("count", err=str(exc), collection=collection, database=database)
            return None
    ...
    def insert(self, database, collection, set_object):
        try:
            ...
            self.mylog.debug("insert", err=str(exc), collection=collection, database=database)
            return None
